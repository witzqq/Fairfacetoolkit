...
def get_img2lmk(pts_file):
    ...
    with open(pts_file, 'r') as f:
        for line in f:
            ...
            lmk = np.array([float(x) for x in line[1:]], dtype=np.float32)  # 修改
            lmk = lmk.reshape((5, 2))
            filename = line[0]
...
def main():
    # 给定tfrecord文件名称，创建生成的tfrecord文件存储位置
    args = parse_args()
    tfrecords_dir = os.path.join('./', args.tfrecords_name)
    print(tfrecords_dir)  # add
    ...
    idx_writer = open(os.path.join(tfrecords_dir, "%s.index" % tfrecords_name), 'w')  # 写入索引文件，文件为idx_writer
    with open(args.img_list, 'r') as f:
        for line in f:
            ...
            img_bytes = cv2.imencode('.jpg', crop_img)[1].tobytes()

            example = tf.train.Example(features=tf.train.Features(feature={
                    'image': tf.train.Feature(bytes_list=tf.train.BytesList(value=[img_bytes]))}))

            if cur_shard_size == 0:  # cur_shard_size的作用？
                print("{}: {} processed".format(dt.now(), count))
                cur_shard_idx += 1
                record_filename = '{0}-{1:05}.tfrecord'.format(tfrecords_name, cur_shard_idx)   # 写入record文件
                print(record_filename)  # add
                if cur_shard_writer is not None:
                    cur_shard_writer.close()
                cur_shard_path = os.path.join(tfrecords_dir, record_filename)
                cur_shard_writer = tf.compat.v1.python_io.TFRecordWriter(cur_shard_path)   # record文件第一项，文件为cur_shard_writer
                cur_shard_offset = 0

            example_bytes = example.SerializeToString()  # cur_shard_writer接受的是一个string,所以需要先将example序列化为string类型
            # print(example_bytes)  # add
            cur_shard_writer.write(example_bytes)  # record文件第二项，文件为cur_shard_writer
            cur_shard_writer.flush()
            idx_writer.write('{}\t{}\t{}\n'.format(img_path, cur_shard_idx, cur_shard_offset))  # idx_writer文件写入项：图片路径，索引、offset
