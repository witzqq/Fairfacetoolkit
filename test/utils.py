...
def perform_rfw_val_bin(data_set, model, device, batch_size=64, nfolds=10):
    ...
    with torch.no_grad():
        for i in range(len(data_list)):
            ...
            while ba < data.shape[0]:
                bb = min(ba + int(batch_size), data.shape[0])
                count = bb - ba
                _data = torch.tensor(data[bb - int(batch_size):bb, ...]).cuda()
                _embeddings = model(_data).cpu().numpy()
                if embeddings is None:
                    embeddings = np.zeros((data.shape[0], _embeddings.shape[1]))
                embeddings[ba:bb, :] = _embeddings[(int(batch_size) - count):, :]
                ba = bb
    ...
    for embed in embeddings_list:
        for i in range(embed.shape[0]):
            _em = embed[i]
            _norm = np.linalg.norm(_em)  # 求范数
            _xnorm += _norm
            _xnorm_cnt += 1
    _xnorm /= _xnorm_cnt  # 平均范数

    acc1 = 0.0
    std1 = 0.0
    # print('\n经主干网络特征提取后输出结果：\n\n\n')  # add
    # print(embeddings_list)  # add
    embeddings = embeddings_list[0] + embeddings_list[1]
    # print(embeddings[0])  # add
    # print(embeddings[0])  # add
    embeddings = sklearn.preprocessing.normalize(embeddings)  # 归一标准化
