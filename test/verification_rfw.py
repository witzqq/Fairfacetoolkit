...
from torchkit.head import get_head  # add
from utils import perform_rfw_val_bin


def parse_args():
    parser = argparse.ArgumentParser(description='verfication tool')
    parser.add_argument('--ckpt_path_backbone', default=None, required=True, help='model_path')

    # parser.add_argument('--ckpt_path_head', default=None, required=True, help='head_path')  # add

    parser.add_argument('--backbone', default='IR_34', help='backbone type')
    # parser.add_argument('--head', default='cifp', help='head type')  # head网络
...
def main():
    ...
    if not os.path.exists(args.ckpt_path_backbone):
        raise RuntimeError("%s not exists" % args.ckpt_path_backbone)
    backbone.load_state_dict(torch.load(args.ckpt_path_backbone))
    # backbone to gpu
    ...
    # load head(add)
    # head = get_head(args.head)(args.embedding_size, class_num)
    # if not os.path.exists(args.ckpt_path_head):
    #     raise RuntimeError("%s not exists" % args.ckpt_path_head)
    # head.load_state_dict(torch.load(args.ckpt_path_head))
    # # head to gpu
    # gpus = [int(x) for x in args.gpu_ids.rstrip().split(',')]
    # if len(gpus) > 1:
    #     head = torch.nn.DataParallel(head, device_ids=gpus)
    # head = head.cuda()
